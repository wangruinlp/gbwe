...
//	int c;
  ...
  char *pd;
	...
	// ****************************************************************
	//   ouverture et lecture des fichier "*.clis" "*.clic" et "*.trad" 
 ...
 //lecture fichier des requetes
	...
	fclose(freq);
		...
		for(i=0;i<nsyns;i++)
		{
  		...
  		sprintf(syncname, "%s.sync", argv[1]);
		...
		fclose(ftras);
		fclose(fsync);
  		sprintf(clicname, "%s.clic", argv[1]);
		...
		fclose(fcoord);
  		...
  		sprintf(coordname, "%s.cpcc", argv[1]);
		fcoord=fopen(coordname,"w");
		fprintf(fcoord,"%f %f",0.0,0.0);
		fclose(fcoord);
		...
		fclose(fcoord);
  		...
  		sprintf(coordname, "%s.cpsc", argv[1]);
		fcoord=fopen(coordname,"w");
		fprintf(fcoord,"%f %f",0.0,0.0);
		fclose(fcoord);
	...
	fclic = ouvre_fichier(argv[1],".clic", "r");
...
//  analyse_fact(nf+1, coord_clis.i, coord_clic.i,matrice_travail,cliS,cliC);
  ...
  ecrit_fichier(argv[1],".cpcc", "w",cliC,coord_clic.i,nf+1);
  ...
  ecrit_fichier(argv[1],".cpsc", "w",synsC,coord_clic.j,nf+1);
