...
int getSynLine_path(const char* word,char* line,const char *base, const char *base_path) //copie dans la chaine 'line', la ligne du dictionnaire associée au mot 'word'
// retourne un nombre positif en cas de succes, negatif en cas d'echec
{
        FILE* findex;
        char num[256]; //temporaire : contient un nombre sous forme de chaine de caratères
        ...
        sprintf(filename,"../../cgi-bin/%s/index.arb.%s",base_path,base);
	        findex=fopen(filename,"rb");
        if (findex==NULL)
			{//printf("\n pas d'index dicoscan");
			return -1;}
        ...
        //lecture de 'line' dans le dico
        sprintf(filename,"../../cgi-bin/%s/vs_dicos.%s",base_path,base);
        findex=fopen(filename,"rb");
        if (findex==NULL)
        	return -2;
        fseek(findex,dicopos,0);
